...
- [ ] **Content gap found while linking travel rules — worth fixing in the writing, not the markup.** Two specific findings:
  1. **No travel article mentions tax residency at all.** Zero matches for "tax resident/tax residency/taxable" across all ten. A visitor-visa reader stacking 180 days in Thailand or 183 in Indonesia crosses a tax-residency threshold, and the article never says so. This is the single clearest cross-category link the library is missing, and it is Immio's whole premise — the app counts days for both.
  2. **`us-b1b2-visa` and `us-esta` never reference each other.** They are the two routes for the same trip; the B-1/B-2 page gets a link via "visa-free entry", but ESTA has no reciprocal mention.
  Both need an author, not a regex — a sentence or an edge-case bullet each, following [ai/Rule Generation Plan.md](ai/Rule%20Generation%20Plan.md).
...
- [ ] P1-1d Back-fill `relatedContent` across the remaining 33 tax rules
